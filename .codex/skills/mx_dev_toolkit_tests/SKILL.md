...
- `src/http_client/tests/register.test.ts`
  - 验证 HTTP Client 状态栏与命令入口不会主动弹出 OutputChannel.
...
- `webviews/http_client/tests/component_contract.test.ts`
  - 验证 React 工作台与侧边栏关键按钮接线, 防止压测入口和管理动作回退.

### 2.2 Selection

- `src/selection/tests/path_format.test.ts`
  - 验证路径范围复制会统一使用正斜杠, 且 Windows 绝对路径盘符固定输出为大写.
...
4. 生成以下日志文件:
   ...
   - `logs/mx-dev-toolkit/tests/http_client_register.txt`
   ...
   - `logs/mx-dev-toolkit/tests/http_client_component_contract.txt`
   - `logs/mx-dev-toolkit/tests/selection_path_format.txt`
...
4. 将新增测试加入对应脚本:
   - 宿主侧加入对应模块脚本, 如 `test:http-client:core` 或 `test:selection`
   - React Webview 加入 `test:http-client:webview`
   - 汇总校验保持接入 `test` 或对应聚合脚本
5. 更新本 Skill 中的测试清单和验收标准.
6. 执行对应测试命令, 如 `pnpm test:http-client` 或 `pnpm test:selection`, 验证新增测试可运行.
